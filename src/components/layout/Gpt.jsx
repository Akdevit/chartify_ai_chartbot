...
const Gpt = ({ showsidebarclick, hidesidebar }) => {
    ...
    const { inputValue, messages, loading, copytextIndex } = useSelector(
        (state) => state.chat
    );
    ...
    const handleSend = () => {
        ...
        dispatch(addMessage({ type: 'user', content: userMessage }));
        ...
        fetch(
            ...
            .then((data) => {
                ...
                dispatch(addMessage({ type: 'bot', content: responseText }));
    ...
    const copytextfunction = (text, index) => {
        ...
        dispatch(setCopytextIndex(index));
        setTimeout(() => {
            dispatch(setCopytextIndex(null));
        }, 2000);
    };
        ...
        <div className="w-[100%] h-[100vh] bg-[#212121] flex flex-col justify-between">
            ...
            <div className="w-full h-[60px] flex justify-between pr-4 pl-4 items-center">
                <div className="w-auto h-auto flex items-center gap-3 justify-center">
                    {!hidesidebar && (
                        <div
                            onClick={showsidebarclick}
                            className="w-[40px] h-[40px] bg-gray-600 rounded-full flex justify-center items-center cursor-pointer"
                        >
                    ...
                    <div
                        ...
                        className={
                            messages.length !== 0
                                ? 'w-[35px] h-[35px] bg-red-600 rounded-full flex justify-center items-center cursor-pointer'
                                : 'w-[35px] h-[35px] bg-gray-600 rounded-full flex justify-center items-center cursor-pointer'
                        }
                        title="Clear Chat"
                    >
            ...
            <div
                ref={chatRef}
                className="w-full h-[100%] flex justify-center overflow-auto scrollbar mb-2"
            >
                    ...
                    {messages.length === 0 ? (
                        <div className="w-full h-[100%] flex flex-col items-center p-4 gap-4">
                            ...
                            <p className="text-white text-center">
                                Hello, I'm Chatify. How can I help you today?
                            </p>
                            <div className="w-full h-auto flex flex-wrap justify-center gap-4">
                                {[
                                    'What are the major causes of climate change?',
                                    'How can I support mental health in my community?',
                                    'How can I contribute to social justice initiatives?',
                                    'What are some strategies for achieving financial literacy?',
                                    'How can I improve my cybersecurity practices?',
                                    'What are some ways to promote sustainable living?',
                                ].map((text, index) => (
                                    <div
                                        key={index}
                                        onClick={() => dispatch(setInputValue(text))}
                                        className="p-1 w-[200px] h-auto rounded-lg border cursor-pointer flex justify-center items-center hover:bg-[#2F2F2F] duration-150"
                                    >
                        ...
                        messages.map((message, index) => (
                            <div
                                key={index}
                                className={`w-full h-auto flex gap-2 ${message.type === 'user'
                                    ? 'justify-end'
                                    : 'justify-start xl:flex-row lg:flex-row md:flex-row  flex-col'
                                    }`}
                            >
                                {message.type === 'bot' && (
            ...
            <div className="w-full h-[80px] flex justify-center">
                <div className="xl:w-[60%] lg:w-[80%] md:w-[80%] w-[95%] h-[50px] rounded-3xl bg-[#2F2F2F] flex justify-around items-center">
                    ...
                    <div
                        ...
                        className={
                            inputValue
                                ? `w-[40px] h-[40px] rounded-full bg-white flex justify-center items-center duration-75 cursor-pointer`
                                : 'w-[40px] h-[40px] rounded-full bg-gray-600 flex justify-center items-center duration-75 cursor-pointer'
                        }
                    >
                        {loading ? (
